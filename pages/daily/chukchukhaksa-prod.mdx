...
- "오류가 발생했습니다. (오류 코드: ERR-1023)" 같은 형식으로 변경
- 메시지는 짧고 간단하게, 어디에 문의해야 하는지 정도만 안내

**오류 코드 시스템 도입**

- 아직 서비스 초기라 모든 오류 코드를 정의하진 못했지만 차차 정의해 나갈 계획이다.
- 사용자에게 모든 기술적 세부 정보를 설명할 필요는 없지만, CS 할 때 빠르게 문제를 파악할 수 있는 고유 코드를 노출
- 예를 들어 ERR-1023은 "학부 코드 누락 케이스"로 내부적으로 바로 인지할 수 있게 매핑

**행동 지침 추가**

- "문제가 반복될 경우 캡처 화면과 학번을 고객센터(사실상 저자)로 문의해주세요"
...
- 예외 객체나 에러 핸들링 로직에 고유 "에러 코드"를 부여
- 예: `ErrorDepartmentGroupMissing` → `ERR-1023`
- 이 코드로 CS 할 때 빠르게 로그를 조회하거나 필터링할 수 있게 함

#### 3. 비즈니스 정책 설명은 최소화, 지원은 충분히

사용자 입장에서는 "왜 에러가 났는지"보다 **지금 당장 내 문제를 어떻게 해결하면 되는지**가 훨씬 중요하다고 판단했다.

사용자가 앞으로 "알 수 없는 에러"라는 막연한 메시지 대신 "무언가 오류가 났지만, 여기 적힌 코드로 문의하면 해결될 거야" 정도로 인지할 수 있게 되면 좋겠다 결과적으로 불필요한 CS 반복 문의도 크게 줄일 수 있길 바란다.
