---
title: "1인 개발, 3일 만에 400명 가입, 척척학사 런칭 과정에서 배운 것들"
date: "2025-03-01"
...
글을 시작하기 앞서 이해를 돕기위해 간단하게 서비스에 대해 소개하겠다.
...
>
  <div style={{ width: "100%" }}>
    ![cchaksa](../../public/assets/daily/images/29.png "척척학사 서비스 소개")
  </div>
  <div style={{ width: "100%" }}>
    ![cchaksa](../../public/assets/daily/images/30.png "척척학사 서비스 소개")
  </div>
</div>

척척학사는 대학생들의 졸업 요건을 자동으로 계산해주는 서비스다. 사용자는 학교 포털 계정을 연동하면 수강 과목, 성적, 학점 이수 내역을 가져와 졸업 조건을 한눈에 확인할 수 있다

**주요 기능**

- ✅ 수강 & 성적 데이터 크롤링
  학교 포털에서 사용자의 학업 정보를 안전하게 수집
- ✅ 졸업 요건 자동 계산
  입학 연도와 학과별 기준을 정확히 적용하여 분석
- ✅ 이수 현황 시각화
  부족한 학점과 충족된 항목을 직관적으로 확인

기존에는 졸업 요건을 확인하려면 엑셀로 정리하고 수작업으로 계산해야 했다. 척척학사를 통해 이 과정을 자동화하여 학생들이 졸업 계획을 더 쉽고 정확하게 세울 수 있게 되었다.

그럼 이제 **실제 배포 후 겪은 문제와 해결 과정**을 공유하려 한다.
...
##### Next.js 13.4+에서 after() 권장

Vercel 공식 문서에서도 Next.js 13.4 이상을 사용 중이라면, after() API를 권장한다.
after()는 응답(또는 프리렌더) 이후에 실행될 코드를 등록하므로, Non-Blocking 방식으로 백그라운드 태스크를 수행할 수 있다.

**장점:**

- 클라이언트가 곧바로 응답을 받고, after(...) 블록이 백그라운드에서 일정 시간 실행됨
- 대기 시간(응답 지연)이 줄어들고, 로컬과 서버리스 환경 모두에서 깔끔히 동작

**주의:**

- after()가 끝없이 돌 수 있는 것은 아니며, **서버리스 한계 Fluid Compute 등**에 맞춰 일정 시간 후 종료될 수 있음
- Edge vs Node.js 런타임, 플랜이나 지역 설정에 따라 정확히 지원 범위가 다를 수 있음

##### waitUntil vs after()

Vercel은 Fluid Compute가 활성화된 프로젝트에서 waitUntil() API를 사용할 수 있다.
하지만 **Next.js 13.4이상**에서는 **after함수**가 이미 내부적으로 waitUntil()과 유사하게 동작하도록 공식화되어 있으며,
Vercel 문서에서도 **Next.js 13.4+ → after() 권장**이라고 명시한다.
...
export async function POST(req: Request) {
  ...
  // 응답은 즉시 반환하고, 크롤링은 백그라운드에서 실행
  const jsonResponse = NextResponse.json({ taskId, studentInfo: {...} }, { status: 202 });

  after(async () => {
    try {
      // 실제 크롤링 로직 (생략)
      await setTask(taskId, 'completed', { message: '동기화 완료' });
      session.destroy(); // 세션 정리
    } catch (err: any) {
...
만약 크롤링이 더 오래 걸리거나, 대규모 병렬 처리가 필요하다면, 메시지 큐 방식이 모범적이다:

1. `/api/start` 라우트에서 Queue(예: SQS, Upstash QStash 등)에 작업을 넣고, 즉시 202 응답
2. 별도 워커(서버리스 함수나 다른 서버)에서 큐 메시지를 소비하여 크롤링 로직 처리
3. 완료 후 DB/Redis에 결과 저장, 클라이언트는 `/progress` 폴링

이렇게 "요청 라우트"와 "실제 작업"을 완전히 분리하면, 서버리스 타임아웃 걱정 없이 장기간 작업을 진행할 수 있고, 확장성도 높아진다.
다만 초기 설정과 구조가 복잡해질 수 있다.

#### 결론

1. 서버리스 환경에서 "응답을 먼저 보내고, 응답 후에 추가 로직(크롤링 등)을 실행"하려면:

   - (과거) `(async () => {...})()`처럼 "return 뒤 비동기 실행"은 보장되지 않음
   - (현재) **Next.js 13.4이상**에서는 **after 함수**를, 그 이하 버전이거나 다른 프레임워크면 waitUntil() 또는 메시지 큐 등을 고려해야 함

2. Blocking(동기) 처리는 빠른 임시방편이지만, 응답 지연으로 사용자 경험에 차이가 줄 수 있음으로 권장하지 않음

3. 메시지 큐나 백엔드 워커 방식을 쓰면 장시간 작업이나 대규모 스케일도 안정적으로 처리 가능
...
#### 🧐 고민과 해결: 예상치 못한 케이스 대응하기

크롤링이나 외부 시스템 연동에서 가장 어렵고도 피곤한 문제 중 하나가 "실제 환경에서 예상치 못한 케이스"가 끊임없이 나온다는 점이다. 특히 학부/학과 코드는 실제로 유저 데이터가 어떻게 구성되어 있는지 전부 파악하기 어렵기 때문에, 테스트 환경에서 놓치게 되는 경우가 많았다.

**즉각적인 해결책으로는 두 가지 접근법을 적용했다:**

1. **로직 개선**: 학부 코드가 없어도 학과 코드만으로 졸업 요건을 계산할 수 있도록 로직을 수정했다. 이를 통해 학부 코드 누락 문제가 있는 학과 학생들도 서비스를 정상적으로 이용할 수 있게 되었다.

2. **오류 메시지 개선**: 혹시라도 문제가 발생할 경우, 사용자에게 "알 수 없는 오류"가 아닌 더 명확한 오류 메시지를 제공하도록 했다. 이를 통해 사용자가 문제 상황을 이해하고 적절한 조치를 취할 수 있게 되었다.

**앞으로의 대응 계획도 세웠다:**

- 예외 케이스 발생 시 자동으로 로깅 + 알림을 보내도록 설정할 예정이다
- 사용자 제보 프로세스를 더 정교화할 계획: 문의 시 빠르게 로그를 찾아 원인을 파악할 수 있는 시스템을 구축하려 한다
...
**기존 CS 대응 프로세스**
    ...
    1.	사용자가 “뭔지는 모르겠지만 알 수 없는 에러가 발생했다고” 문의
    2.	개발자가 사용자 학번을 받아 Sentry 로그 검색
    3.	Vercel/AWS 로그에서 원인 추적
    4.	문제를 찾아 사용자에게 안내
    5.  문제를 못 찾으면 사용자에게 문제 추정 안내
...
이전 글에서도 언급했지만, supabase 사용하면서 체감한 가장 크게 느낀 장점은 admin이다. 혼자서 프론트와 백엔드를 모두 개발하는 상황에서 이걸 직접 개발할 생각을 했으면 리소스가 꽤나 들었을 것 같다. 두번째로는 천명까진 거뜬한 무료 플랜... 사이드 프로젝트에 이보다 더 적합한 선택지를 찾기 어려웠다.
...
회사에서는 내 코드가 만든 문제를 다른 팀원들이 완충해주고 있었다는 걸 깨달았다. 그리고 개발자의 영역에서도 사용자에게 더 좋은 경험을 줄 수 있도록 설계해야 한다는 생각이 강하게 들었다. 에러 메시지 하나, 로깅 방식 하나가 얼마나 중요한지 몸소 체험하게 된 셈이다
